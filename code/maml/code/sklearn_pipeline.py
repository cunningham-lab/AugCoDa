...
#         plot_tsne(self.X,Y=self.Y,targets=self.target_names, filename=self.filename +'.after_mrmr_feature_selection')
    ...
    @utils.tryExcept
    def select_best_scl_clf(self, SCALERS, Tree_based_CLASSIFIERS, Other_CLASSIFIERS, augmenter,
        ...
        grid_search = GridSearchCV(pipeline, param_grid=param_grid,
                                   scoring = scoring, cv =inner_cv, #error_score='raise',  
        ...
        # grid_search.fit(self.X, self.Y, clf__sample_weight=self.w)# Train the classifier with balanced data ; fit did nothing 
        self.grid_search = grid_search
        self.scoring = scoring
        #通过交叉验证获得预测值
        self.y_pred = cross_val_predict(grid_search, self.X, self.Y, cv=outer_cv, n_jobs=n_jobs) #n_job= -1 会出错，和inver_cv冲突
    ...
    @utils.tryExcept
    def hypertune_best_classifier(self, All_CLASSIFIERS, pltcfm=False, outer_cv=10, inner_cv=5,n_jobs=1):
        ...
        if self.search:
            ...
            best_clf_index = [i[0].__class__.__name__ for i in All_CLASSIFIERS].index(self.best_clf.__class__.__name__)
            param_grid = All_CLASSIFIERS[best_clf_index][1]

            grid_search = GridSearchCV(self.best_estimator_, param_grid=param_grid, 
                               scoring= self.scoring, #[independently identically distributed across the folds] return the average score across folds
            ...
            # grid_search.fit(self.X, self.Y, clf__sample_weight=self.w) # Train the classifier with balanced data
...
#         clf = load(modelf) 
        ...
        if False: # hasattr(best_hypertuned_clf, "feature_importances_"):
...
#             plot_tsne(topk_X,Y=topK_Y,targets=self.target_names, filename=self.filename +'.{}.hypertuned.top{}.important_features'.format(self.clf_name, k))
        ...
        elif False: # hasattr(best_hypertuned_clf, "coef_"):
